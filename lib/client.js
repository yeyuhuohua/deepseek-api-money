window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 谷时: 其余全部时间，价格为峰时的一半
    // 来源: https://api-docs.deepseek.com/zh-cn/quick_start/pricing
    //      DeepSeek 平台公告：「调休上班的周末、中国法定节假日全天均按空闲时段计费」
    //      → 所以判定只看「周几 + 是否法定节假日」，调休上班的周末同样算谷时
    ...
    // 调休上班的周末（官方明确：仍按空闲时段计费，不因为「要上班」而变峰时）
    // 2026 年：元旦 01-04、春节 02-14 / 02-28、劳动节 05-09、国庆 09-20 / 10-10
    var CN_MAKEUP_DAYS = ["2026-01-04", "2026-02-14", "2026-02-28", "2026-05-09", "2026-09-20", "2026-10-10"];
    var CN_MAKEUP_SET = {};
    for (var mi = 0; mi < CN_MAKEUP_DAYS.length; mi++) CN_MAKEUP_SET[CN_MAKEUP_DAYS[mi]] = true;
    ...
    // "peak" | "weekend" | "makeup" | "holiday" | "offpeak"
    function periodOf(date) {
      var bj = beijing(date);
      var dow = bj.getUTCDay(); // 0=周日 6=周六
      var h = bj.getUTCHours();
      var key = bjDateKey(date);
      if (CN_HOLIDAY_SET[key] === true) return "holiday";
      if (dow === 0 || dow === 6) return CN_MAKEUP_SET[key] === true ? "makeup" : "weekend";
